...
void MoveGenerator::clearMoves() { moveCount = 0; }

int MoveGenerator::getLegalMoveCount() const { return moveCount; }
...
std::vector<Move> MoveGenerator::getPieceMoves(int square, const BoardState* board) {
    ...
    switch (pieceType) {
        ...
        case PIECE_PAWN: {
            ...
            // captures
            const auto& pawnAttacks = (friendlyColour == COLOR_WHITE) ? PrecomputedMoveData::getPawnAttacksWhite(pawnSquare) :
...
void MoveGenerator::calculateOpponentAttackData() {
    ...
    const uint64_t checkingPawns = PrecomputedMoveData::getPawnAttackBitboard(opponentColour ^ 1, friendlyKingSquare) & opponentPawnsBb;
    if (checkingPawns) {
        inDoubleCheck = inCheck;
        inCheck = true;
    ...
    if (unconstrained) {
        for (int i = 0; i < rookSquares.count(); ++i) {
            ...
            while (attacks) {
                ...
                if (opponentOccupancy & toMask)
                    addCaptureMove(*board, fromSquare, toSquare);
                else if (genQuiets)
                    addQuietMove(*board, fromSquare, toSquare);
            }
        }
        return;
    }
...
bool MoveGenerator::getInCheck() const { return inCheck; }
}  // namespace Chess
