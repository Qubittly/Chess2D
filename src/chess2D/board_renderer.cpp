...
BoardRenderer::~BoardRenderer() { destroyPieceTextures(); }
...
void BoardRenderer::setBlendModeAlpha() { SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND); }
void BoardRenderer::resetBlendMode() { SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_NONE); }
...
void BoardRenderer::drawSelectedSquareHighlight(int square, const std::vector<Move>& moves) {
    ...
    setBlendModeAlpha();
    if (isLegalMove) {
        SDL_SetRenderDrawColor(renderer, colors.selectedSquare.r, colors.selectedSquare.g, colors.selectedSquare.b,
                               colors.selectedSquare.a);
    } else {
...
void BoardRenderer::drawPieces() {
    ...
    for (int sq = 0; sq < 64; ++sq) {
        ...
        SDL_FRect dst = { squareRect.x + (maxW - renderW) * 0.5f, squareRect.y + (maxH - renderH) * 0.5f, renderW, renderH };

        SDL_RenderTexture(renderer, tex, nullptr, &dst);
    }
}

int BoardRenderer::textureIndex(int color, int pieceType) { return color * 6 + pieceType; }
...
void BoardRenderer::setFlipped(bool flipped) { isFlipped = flipped; }

void BoardRenderer::setColors(int option) { colorOption = std::clamp(option, 1, 3); }
...
void BoardRenderer::ensurePieceTexturesLoaded() {
    ...
    for (int color = 0; color < 2; ++color) {
        for (int type = 0; type < 6; ++type) {
            ...
            if (pieceTexOption == 1 || pieceTexOption == 2)
                SDL_SetTextureScaleMode(pieceTextures[idx], SDL_SCALEMODE_NEAREST);
            else
                SDL_SetTextureScaleMode(pieceTextures[idx], SDL_SCALEMODE_LINEAR);
        }
    }
