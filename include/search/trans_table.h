...
public:
    explicit TranspositionTable(int sizeMB = 50) { resize(sizeMB); }

    ~TranspositionTable() { delete[] table; }

    std::size_t getIndex(uint64_t key) const { return key % tableSize; }
    ...
    void newSearch() { currentAge.fetch_add(1, std::memory_order_relaxed); }
    ...
    static constexpr int getLookupFailedValue() { return std::numeric_limits<int>::min(); }
...
private:
    ...
    static constexpr std::size_t getStripeIndex(std::size_t tableIndex) { return tableIndex & (TT_LOCK_STRIPES - 1); }
