...
class Evaluation {
public:
    explicit Evaluation(const BoardState& board, EvaluationOptions options = {}) : board(board), options(options) {}
    ...
    int EvaluateCastlingAndRookDevelopment() const {
        ...
        auto sidePenalty = [this, phase](int color) {
            ...
            const bool hasKingSideRight = (color == COLOR_WHITE) ? ((rights & 0x01) != 0) : ((rights & 0x04) != 0);
            const bool hasQueenSideRight = (color == COLOR_WHITE) ? ((rights & 0x02) != 0) : ((rights & 0x08) != 0);
    ...
    int EvaluateKingSafety() const {
        auto sideSafety = [this](int color) {
            ...
            for (int f = std::max(0, kingFile - 1); f <= std::min(7, kingFile + 1); ++f) {
                ...
                if (!hasNearShieldPawn)
                    penalty += KING_SHIELD_MISSING_NEAR_PAWN_PENALTY;
                else if (!hasFarShieldPawn)
                    penalty += KING_SHIELD_MISSING_FAR_PAWN_PENALTY;
            ...
            for (int i = 0; i < oppPawnList.count(); ++i) {
                ...
                if (color == COLOR_WHITE) {
                    if (rank <= 2)
                        penalty += KING_PAWN_STORM_NEAR_PENALTY;
                    else if (rank == 3)
                        penalty += KING_PAWN_STORM_MID_PENALTY;
                    else if (rank == 4)
                        penalty += KING_PAWN_STORM_FAR_PENALTY;
                } else {
                    if (rank >= 5)
                        penalty += KING_PAWN_STORM_NEAR_PENALTY;
                    else if (rank == 4)
            ...
            if (options.useVirtualKingMobility) {
                ...
                const uint64_t virtualQueenAttacks = (bishopAttacks(kingSq, occNoKing) | rookAttacks(kingSq, occNoKing)) & ~ownOccNoKing;
                ...
                const int enemySliders = board.getPieceList(opponent, PIECE_BISHOP).count() +
    ...
    int EvaluatePawnStructure() const {
        auto evaluateSide = [this](int color) {
            ...
            uint64_t stops = (color == COLOR_WHITE) ? (pawns << 8) : (pawns >> 8);
            ...
            uint64_t backwardPawns = (color == COLOR_WHITE) ? (backwardStops >> 8) : (backwardStops << 8);
    ...
    int EvaluateMobility() const {
        ...
        auto sideMob = [this, phase](int color) {
            ...
            const uint64_t allPawns = board.getPieceBoards()[COLOR_WHITE][PIECE_PAWN] | board.getPieceBoards()[COLOR_BLACK][PIECE_PAWN];
            ...
            const uint64_t kingRing = (enemyKingSq >= 0) ? (PrecomputedMoveData::getKingMoves(enemyKingSq) & ~allPawns) : 0ULL;
    ...
    std::array<int, 64> getInterpolatedPieceTable(int pieceType, int color, int phase) const {
        ...
        switch (pieceType) {
            case PIECE_PAWN:
                mgBase = &MG_PAWN_TABLE;
                egBase = &EG_PAWN_TABLE;
                break;
            case PIECE_KNIGHT:
                mgBase = &MG_KNIGHT_TABLE;
                egBase = &EG_KNIGHT_TABLE;
                break;
            case PIECE_BISHOP:
                mgBase = &MG_BISHOP_TABLE;
                egBase = &EG_BISHOP_TABLE;
                break;
            case PIECE_ROOK:
                mgBase = &MG_ROOK_TABLE;
                egBase = &EG_ROOK_TABLE;
                break;
            case PIECE_QUEEN:
                mgBase = &MG_QUEEN_TABLE;
                egBase = &EG_QUEEN_TABLE;
                break;
            case PIECE_KING:
                mgBase = &MG_KING_TABLE;
                egBase = &EG_KING_TABLE;
                break;
            default: return table;
        }
    ...
    void setOptions(const EvaluationOptions& newOptions) { options = newOptions; }

    const EvaluationOptions& getOptions() const { return options; }
...
private:
    ...
    uint64_t mobilityMask(int color, uint64_t occ) const {
        const int opponent = color ^ 1;
        const uint64_t ownPawns = board.getPieceBoards()[color][PIECE_PAWN];
        const uint64_t blockedPawns = (color == COLOR_WHITE) ? (ownPawns & (occ >> 8)) : (ownPawns & (occ << 8));
    ...
    uint64_t getStragglerPawns(int color) const {
        ...
        uint64_t backwardPawns =
            (color == COLOR_WHITE) ? ((stops & oppPawnAttacks & ~ownAttackSpans) >> 8) : ((stops & oppPawnAttacks & ~ownAttackSpans) << 8);

        // Get open pawns
        uint64_t openPawns = getOpenPawns(color);

        // Early ranks: rank 2-3 for white (0x00FF0000), rank 6-5 for black (0x0000FF00)
        uint64_t earlyRankMask = (color == COLOR_WHITE) ? 0x0000FF00ULL  // ranks 1-2 (bits 8-15)
                                                          :
    ...
    static uint64_t northOne(uint64_t bb) { return bb << 8; }

    static uint64_t southOne(uint64_t bb) { return bb >> 8; }

    static uint64_t eastOne(uint64_t bb) { return (bb << 1) & ~FILE_A; }

    static uint64_t westOne(uint64_t bb) { return (bb >> 1) & ~FILE_H; }

    static uint64_t pawnsWithEastNeighbors(uint64_t pawns) { return pawns & westOne(pawns); }

    static uint64_t pawnsWithWestNeighbors(uint64_t pawns) { return pawns & eastOne(pawns); }
    ...
    int getTotalPawnCount() const {
        return popCount(board.getPieceBoards()[COLOR_WHITE][PIECE_PAWN] | board.getPieceBoards()[COLOR_BLACK][PIECE_PAWN]);
    }

    int getCentralRamCount() const { return popCount((getPawnRams(COLOR_WHITE) | getPawnRams(COLOR_BLACK)) & EXTENDED_CENTER_MASK); }
